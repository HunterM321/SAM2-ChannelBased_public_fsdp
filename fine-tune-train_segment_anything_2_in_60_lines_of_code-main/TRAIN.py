...
csv_file = r"/home/hunter.ma/SAM2-ChannelBased_public/data/train.csv"  # Replace with your CSV file path
...
lora_checkpoint = torch.load('/home/hunter.ma/SAM2-ChannelBased_public/prev_results/test-results-whole-image/model.torch')
print("loaded model")
...
num_epochs = 50
num_iterations_per_epoch = len(data)  # Assuming `data` is your dataset
...
for epoch in range(num_epochs):
    ...
    with tqdm(total=num_iterations_per_epoch, desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
        for itr in range(num_iterations_per_epoch):
            with torch.cuda.amp.autocast():  # cast to mixed precision
                ...
                scaler.update()  # Mix precision update
    ...
    print(f"Epoch {epoch+1}/{num_epochs} completed with Mean IOU: {mean_iou}")
